...
class Graph:
    ...
    def bfs(self, begin_id: str, end_id: str) -> Tuple[List[str], float]:
        ...
        :param begin_id: The vertex that has the initial id in which you start out the algorithm with
        :param end_id: The vertex that has the end id in which the algorithm ends at 
        :return: Returns tuple of the form (path, distance) where
        path is a list of vertex id strings beginning with begin_id,
        terminating with end_id, and including the ids of all intermediate vertices connecting the two
        distance is the sum of the weights of the edges along the path traveled
        """
        self.add_to_graph(begin_id)
        self.add_to_graph(end_id)

        queue = SimpleQueue()
        queue.put(begin_id)
        back_edges = {begin_id: None}
        visited = set([begin_id])
        ...
        while not queue.empty():
            ...
            for neighbor in self.vertices[current_id].adj:
                if neighbor not in visited:
                    back_edges[neighbor] = current_id
                    queue.put(neighbor)
    ...
    def a_star(self, start_id: str, target_id: str, metric: Callable[[Vertex, Vertex], float]) -> Tuple[List[str], float]:
        """
        Perform an A* search beginning at vertex with id start_id and terminating at vertex with id end_id
        :param start_id: The vertex that has the initial id in which you start out the algorithm with
        :param target_id: The vertex that has the target id in which the algorithm ends at 
        :param metric: Use the metric parameter to estimate h(v), the remaining distance, at each vertex
        :return: Returns tuple of the form ([path], distance) where
        [path] is a list of vertex id strings beginning with start_id,
        terminating with end_id, and including the ids of all intermediate vertices connecting the two
        distance is the sum of the weights of the edges along the [path] traveled
        """
        start_vertex = self.get_vertex_by_id(start_id)
        target_vertex = self.get_vertex_by_id(target_id)

        # Check if start_vertex or target_vertex is None
        if start_vertex is None or target_vertex is None:
            return [], 0

        open_queue = AStarPriorityQueue()
        g_values = {start_id: 0}
        back_edges = {start_id: None}

        open_queue.push(0, start_vertex)

        while not open_queue.empty():
            current_priority, current_vertex = open_queue.pop()

            if current_vertex.id == target_id:
                return self.build_path(back_edges, start_id, target_id)

            for neighbor_id in current_vertex.adj:
                tentative_g_value = g_values[current_vertex.id] + self.get_edge_by_ids(current_vertex.id, neighbor_id)[2]

                if neighbor_id not in g_values or tentative_g_value < g_values[neighbor_id]:
                    g_values[neighbor_id] = tentative_g_value
                    f_value = tentative_g_value + metric(self.get_vertex_by_id(neighbor_id), target_vertex)
                    back_edges[neighbor_id] = current_vertex.id

                    neighbor_vertex = self.get_vertex_by_id(neighbor_id)
                    if neighbor_vertex.id in open_queue.locator:
                        open_queue.update(f_value, neighbor_vertex)
                    else:
                        open_queue.push(f_value, neighbor_vertex)
    ...
    def tollways_algorithm(self, start_id: str, target_id: str, coupons: int) -> Tuple[float, float]:
        """
        Perform a toll algorithm beginning at vertex with id start_id and terminating at vertex with id target_id
        :param start_id: The vertex that has the initial id in which you start out the algorithm with
        :param target_id: The vertex that has the end id in which the algorithm ends at 
        :param coupons: The coupon parameter is the maximum number of coupons
        :return: Returns tuple of the form (cost, coupons) where
        cost is the cost of the best path beginning with start_id, terminating with target_id
        ...
        open_queue = TollWayPriorityQueue()
        g_values = {(start_id, c): (float('inf'), 0) for c in range(coupons + 1)}
        g_values[(start_id, 0)] = (0, 0)  # Start from 0 coupons used

        open_queue.push(0, 0, start_vertex)  # Push the start vertex with 0 coupons used

        while not open_queue.empty():
            current_priority, current_coupons, current_vertex = open_queue.pop()

            if current_vertex.id == target_id:
                return g_values[(current_vertex.id, current_coupons)]

            for neighbor_id in current_vertex.adj:
                edge_weight = self.get_edge_by_ids(current_vertex.id, neighbor_id)[2]
                neighbor_vertex = self.get_vertex_by_id(neighbor_id)

                # Try to use a coupon on this edge
                if current_coupons < coupons:  # Check if we have coupons left
                    new_g_value = g_values[(current_vertex.id, current_coupons)][0] + edge_weight // 2
                    new_coupons_used = g_values[(current_vertex.id, current_coupons)][1] + 1
                    if new_g_value < g_values.get((neighbor_id, current_coupons + 1), (float('inf'), 0))[0]:
                        g_values[(neighbor_id, current_coupons + 1)] = (new_g_value, new_coupons_used)
                        if (neighbor_vertex.id, current_coupons + 1) in open_queue.locator:
                            open_queue.update(new_g_value, current_coupons + 1, neighbor_vertex)
                        else:
                            open_queue.push(new_g_value, current_coupons + 1, neighbor_vertex)

                # Don't use a coupon on this edge
                new_g_value = g_values[(current_vertex.id, current_coupons)][0] + edge_weight
                new_coupons_used = g_values[(current_vertex.id, current_coupons)][1]
                if new_g_value < g_values.get((neighbor_id, current_coupons), (float('inf'), 0))[0]:
                    g_values[(neighbor_id, current_coupons)] = (new_g_value, new_coupons_used)
                    if (neighbor_vertex.id, current_coupons) in open_queue.locator:
                        open_queue.update(new_g_value, current_coupons, neighbor_vertex)
                    else:
                        open_queue.push(new_g_value, current_coupons, neighbor_vertex)
